...
//go:embed templates/synced.gen.go
...
Commands:
  ...
  sync    Update .sage/synced.gen.go and sync workflows to .github/workflows`)
}
...
func runInit(args []string) error {
	...
	if _, err := os.Stat(".sage/sagefile.go"); err == nil {
		return errors.New(".sage/sagefile.go already exists, use 'sync' to update synced.gen.go")
	}
	...
	if err := os.WriteFile(".sage/synced.gen.go", []byte(syncedContent), 0o644); err != nil {
		return fmt.Errorf("write .sage/synced.gen.go: %w", err)
	}

	fmt.Println("Initialized .sage/sagefile.go and .sage/synced.gen.go")
...
func runSync(args []string) error {
	...
	// Update synced.gen.go if .sage directory exists
	if _, err := os.Stat(".sage"); err == nil {
		if err := os.WriteFile(".sage/synced.gen.go", []byte(syncedContent), 0o644); err != nil {
			return fmt.Errorf("write .sage/synced.gen.go: %w", err)
		}
		fmt.Println("Updated .sage/synced.gen.go")
	}
